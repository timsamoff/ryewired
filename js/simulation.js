// ── Simulation Engine ─────────────────────────────────────────────────────────
// Behavioral nodal analysis at ~100Hz control rate.
// Row encoding: 0-9 = main grid rows a-j, 'rtp'/'rtm'/'rbp'/'rbm' = rail rows.
...
const Simulation = (() => {
  ...
  function tick() {
    ...
    const Vsupply = supply ? parseFloat(supply.props.voltage) || 9 : 0;

    for (const inst of placed) {
      if (inst.failed) continue;
      const def = ComponentRegistry.getById(inst.defId);
      if (!def) continue;
      try { solveComponent(inst, def, Vsupply, netMap, placed); }
  ...
  function solveComponent(inst, def, Vsupply, netMap, placed) {
    switch (def.behavior?.type) {
      ...
      case 'resistor': {
        ...
        inst._current = Vsupply / R;
        inst._voltage = Vsupply;
        const P = inst._voltage * inst._current;
      ...
      case 'led': {
        ...
        const ImaxA   = (parseFloat(inst.props.max_current_ma) || 20) / 1000;
        const seriesR = placed.find(p => p.defId === 'resistor' && !p.failed);
        const R       = seriesR ? parseFloat(seriesR.props.resistance) || 470 : 0;
        const I       = R > 0 ? (Vsupply - Vf) / R : ImaxA * 3;
        inst._current    = I;
        inst._brightness = Utils.clamp(I / ImaxA, 0, 1);
        if (I > ImaxA * (def.failure_modes?.over_current?.threshold_multiplier || 1.5)) {
          fail(inst, def, 'over_current');
        }
        break;
      }
      ...
      case 'potentiometer': {
        ...
        inst._rLow  = Rt * pos;
        inst._rHigh = Rt * (1 - pos);
        inst._voltage = Vsupply * pos;
        break;
      }

      case 'bjt_npn': {
        const mk     = inst.props.model || '2N3904';
        const params = def.model_params?.[mk] || {};
        const hfe    = parseFloat(inst.props.hfe) || params.hfe || 100;
        const Vbe    = params.vbe || 0.65;
        const Ib     = Vsupply > Vbe ? (Vsupply - Vbe) / 10000 : 0;
        const Ic     = hfe * Ib;
        const IcMax  = (params.max_ic_ma || 200) / 1000;
        inst._current = Ic;
      ...
      case 'capacitor': {
        ...
        if (Vsupply > vr * 1.1) fail(inst, def, 'over_voltage');
        inst._voltage = Vsupply;
        break;
      }
  ...
  // ── Net map (union-find) ────────────────────────────────────────────────────
  // Holes in the same column AND same half (a-e or f-j) are internally connected.
  // Rail holes in the same rail row are connected (with the break at col 31).
  ...
  function buildNetMap(placed, wires) {
    ...
    // Internal column connections: rows 0-4 (a-e) connected vertically per col
    for (let col = 0; col < 63; col++) {
      for (let r = 1; r <= 4; r++) union(key(0, col), key(r, col));
    }
    // rows 5-9 (f-j) connected vertically per col
    for (let col = 0; col < 63; col++) {
      for (let r = 6; r <= 9; r++) union(key(5, col), key(r, col));
    }

    // Rail horizontal connections (with break at col 31)
    for (const railRow of ['rtp','rtm','rbp','rbm']) {
      // Left segment: 0-30
      for (let col = 1; col <= 30; col++) union(key(railRow, 0), key(railRow, col));
      // Right segment: 31-62
      for (let col = 32; col <= 62; col++) union(key(railRow, 31), key(railRow, col));
    }
