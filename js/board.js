...
const Board = (() => {
  ...
  const DIP_GAP      = 18;
  const LABEL_PAD    = 8;
  const LEG_HIT_R    = 10;   // larger hit radius for leg targets
  const LEG_DOT_R    = 6;    // visible dot radius
  const WIRE_HIT_W   = 7;
  ...
  // Drag state machine
  // mode: idle | comp-pending | comp-dragging | leg-dragging
  let _dragMode='idle';
  let _dragInst=null;
  let _dragLegIdx=-1;
  let _dragAnchorLeg=null;   // fixed leg hole during leg drag
  let _dragStartX=0, _dragStartY=0;
  let _dragOffsetX=0, _dragOffsetY=0; // pixel offset of body from origin during free drag
  let _savedLegs=null;
  ...
  // During free body drag, use pixel offset instead of leg holes
  function instGeometryDrag(inst) {
    ...
    const ang=Math.atan2(b.y-a.y, b.x-a.x);
    const len=Math.hypot(b.x-a.x, b.y-a.y);
    return {cx,cy,ang,len,a:{x:a.x+_dragOffsetX,y:a.y+_dragOffsetY},b:{x:b.x+_dragOffsetX,y:b.y+_dragOffsetY}};
  }
  ...
  function hitTestLeg(x,y) {
    ...
    for (let i=0;i<inst.legs.length;i++) {
      // Skip center leg of 3-leg components entirely
  ...
  function drawComponents(c) {
    for (const inst of _placed) {
      const isBeingDragged=(_dragMode==='comp-dragging'&&inst===_dragInst);
      drawInst(inst,c,isBeingDragged?0.45:1.0,isBeingDragged);
    }
    if (_selectedComp) {
      const inst=_placed.find(p=>p.instanceId===_selectedComp);
  ...
  function drawLegTargets(inst,c) {
    ...
    for (let i=0;i<inst.legs.length;i++) {
      ...
      // Directional arrows (←→) using canvas lines
  ...
  function drawInst(inst,c,alpha,useOffset) {
    ...
    let geo;
    if (useOffset && _dragMode==='comp-dragging') {
      geo=instGeometryDrag(inst);
    } else {
      geo=instGeometry(inst);
    }
    const {cx,cy,ang,len}=geo;
    ...
    if (inst.legs.length===3) {
      ...
      const pcx=(cm.x-cx)*dx2+(cm.y-cy)*dy2;
      ctx.beginPath(); ctx.moveTo(pcx,0); ctx.lineTo(pcx,-ll*1.5); ctx.stroke();
    }
  ...
  function drawLED(hex,bw,bh,brightness) {
    ...
    const r=bw/2, h=bh/2;
    ctx.beginPath();
    // Flat cathode face on left, dome on right
    ctx.moveTo(-r,-h);       // top-left (cathode flat)
    ctx.lineTo(-r,h);        // bottom-left
    ctx.lineTo(r*0.3,h);     // bottom corner toward dome
    ctx.arc(0,0,r,Math.PI*0.5,-Math.PI*0.5,true); // dome arc on right
    ctx.lineTo(r*0.3,-h);    // top corner
    ctx.closePath();
    ctx.fillStyle=brightness>0.05?hex:hex+'88'; ctx.fill();
    ctx.strokeStyle='rgba(0,0,0,0.4)'; ctx.lineWidth=0.8; ctx.stroke();
    // Cathode flat-face indicator line
    ctx.strokeStyle='rgba(255,255,255,0.6)'; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(-r,-h); ctx.lineTo(-r,h); ctx.stroke();
    // Polarity symbols
    ctx.font='bold 7px monospace'; ctx.textAlign='center';
    ctx.fillStyle='rgba(255,255,255,0.7)';
    ctx.fillText('+',r*0.35,3);
    ctx.fillStyle='rgba(255,255,255,0.5)';
    ctx.fillText('–',-r*0.5,3);
  }
  ...
  // ── Palette ghost — draws actual component preview ────────────────────────────
  function drawPaletteGhost(mx,my,c) {
    if (!_paletteGhost) return;
    const def=ComponentRegistry.getById(_paletteGhost.defId);
    if (!def) return;

    // Create a fake inst for rendering
    const fakeInst={
      defId: def.id,
      legs:[{row:3,col:5},{row:3,col:5+(def.leg_span||2)-1}],
      flipped:false, props:{}, failed:false,
      _brightness:0, _state:false,
    };
    // Copy default props
  ...
  function onMouseMove(e) {
    ...
    if (_dragMode==='comp-pending') {
      if (Math.hypot(x-_dragStartX,y-_dragStartY)>DRAG_THRESHOLD) {
        _dragMode='comp-dragging';
        document.body.classList.add('dragging');
      }
    }

    if (_dragMode==='comp-dragging') {
      // Free pixel movement — just track offset
      _dragOffsetX=x-_dragStartX;
      _dragOffsetY=y-_dragStartY;
    }
  ...
  function onMouseUp(e) {
    ...
    if (_dragMode==='comp-dragging') {
      // Snap each leg to nearest hole, or restore if no valid hole nearby
      if (_dragInst&&_savedLegs) {
        _dragInst.legs=_savedLegs.map(l=>{
          const {x:lx,y:ly}=holeToXY(l.row,l.col);
          const nearestHole=xyToHole(lx+_dragOffsetX,ly+_dragOffsetY);
          return nearestHole||l;
        });
      }
  ...
  function loadLayout(layout) {
    _placed=layout.components||[]; _wires=layout.wires||[]; setSelected(null,null); render();
  }
  ...
  function getLayoutData() {
    return {
      components:_placed.map(inst=>{
        ...
        delete c._voltage;delete c._current;delete c._audioNode;delete c._brightness;delete c._state;return c;
      }),
      wires:_wires
    };
  }
